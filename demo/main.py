...
def main():
    ...
    # Instantiate Model
    model = ImprovedTinyVGGModel(input_shape=3, hidden_units=48, output_shape=6).to(device)
    ...
    if uploaded_file is not None:
        ...
        predicted_label, image_pred_probs = predict_image(model, custom_image_transformed, class_names)

        # Prediction result section
        st.markdown('<h3 style="color: green;">Prediction Result</h3>', unsafe_allow_html=True)
